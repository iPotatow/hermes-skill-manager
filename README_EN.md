...
- The Optional inventory reads Hermes' bundled official catalog and supports search, category filters, and direct installation; installed entries have the same reset, delete, and Codex sync actions as built-ins; descriptions follow the Desktop language with a complete bundled Chinese snapshot
- A weekly GitHub Actions workflow synchronizes both built-in and Optional official Chinese docs; official translations win, manual fallbacks survive gaps, and missing Chinese blocks an invalid snapshot commit
...
- Built-in, installed Optional, community, and local skills all show the “Sync” action and can be copied into Codex; replacement requires exact-name confirmation
...
- Hermes, Optional, and Codex filesystem paths are hidden from the UI; details retain source and identifier information, with Escape-to-close, focus trapping, and focus restoration
...
.github/workflows/sync-skill-translations.yml  # weekly unified snapshot automation
...
Plugin ID: `skill-manager` · Desktop route: `/skill-manager` · Version: `1.8.5`
