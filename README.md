...
- Hermes、千问办公、WorkBuddy 与 Codex 技能均使用紧凑表格展示，不使用技能卡片
- Hermes、千问办公、WorkBuddy 与 Codex 使用一级分段视图；千问办公和 WorkBuddy 在技能数量为 0 时隐藏；内建、社区、本地处于 Hermes 来源筛选的同一级
- 千问办公视图读取 `~/.qwenworkcn/skills`，自动剔除 QwenWork 安装包内建技能，仅展示可管理的自定义/导入技能；支持搜索、分类、详情和安全删除，删除要求输入完整技能名确认
- WorkBuddy 视图读取 `~/.workbuddy/skills`，自动剔除 WorkBuddy 应用内建技能，仅展示可管理的自定义/导入技能；支持搜索、分类、详情和安全删除，删除要求输入完整技能名确认
...
- `plugin_api.py` 只负责 FastAPI 请求适配与错误转换；Hermes、千问办公、WorkBuddy 和 Codex 的发现、路径、文件操作、运行时调用、状态和业务流程分别位于 `dashboard/skill_manager/`。
