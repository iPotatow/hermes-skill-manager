...
function DetailDialog({ language, onAction, onClose, row, t }) {
  ...
  return jsx('div', { className: 'fixed inset-0 z-40 flex justify-end bg-background/90 backdrop-blur-sm', role: 'presentation', onMouseDown: event => event.target === event.currentTarget && onClose(), children:
    jsxs('section', { 'aria-label': t('details'), className: 'flex h-full w-full max-w-lg flex-col border-l border-(--ui-stroke-secondary) bg-background shadow-xl', role: 'dialog', children: [
      ...
      row.availableActions?.length ? jsx('footer', { className: 'flex flex-wrap justify-end gap-2 border-t border-(--ui-stroke-secondary) p-4', children: row.availableActions.map(action => jsx(Button, { variant: action === 'delete' ? 'destructive' : 'secondary', onClick: () => onAction(row, action), children: t(`actions.${action}`) }, action)) }) : null
    ] })
  })
}
...
function SkillManagePage() {
  ...
  const mutation = useMutation({
    mutationFn: ({ action, row, confirm }) => pluginContext.rest(`/${action}`, { method: 'POST', body: { source: sourceOf(row), name: row.name, ...(confirm ? { confirm } : {}) } }),
  ...
  const beginAction = (row, action) => action === 'delete' || action === 'reset' ? setPending({ row, action }) : mutation.mutate({ row, action, confirm: '' })
    ...
    jsxs('div', { className: 'flex h-full min-h-0 flex-col', children: [
      ...
      jsx(ScrollArea, { className: 'min-h-0 flex-1', children: jsxs('main', { className: 'mx-auto w-full max-w-7xl space-y-4 p-4', children: [
        ...
        visible.length ? jsx('section', { className: 'grid gap-2', children: visible.map(row => jsxs('article', { className: 'grid gap-3 rounded-md border border-(--ui-stroke-secondary) p-3 hover:border-(--ui-stroke-primary) md:grid-cols-[minmax(0,1fr)_auto]', children: [
          jsxs('button', { className: 'min-w-0 text-left', type: 'button', onClick: () => setSelected(row), children: [jsx('strong', { className: 'block break-all text-sm font-medium', children: row.name }), jsx('p', { className: 'mt-1 line-clamp-2 break-words text-sm leading-5 text-(--ui-text-secondary)', children: descriptionOf(row, language) || t('noDescription') }), jsxs('div', { className: 'mt-2 flex flex-wrap gap-1.5', children: [jsx(ToneBadge, { tone: sourceOf(row), children: t(`sources.${sourceOf(row)}`) }), jsx(ToneBadge, { tone: row.status, children: t(`statuses.${row.status}`) }), row.category ? jsx(Badge, { className: 'max-w-48 truncate font-normal', children: row.category }) : null] })] }),
          row.availableActions?.length ? jsx('div', { className: 'flex flex-wrap items-center gap-2 md:justify-end', children: row.availableActions.map(action => jsx(Button, { disabled: mutation.isPending, size: 'sm', variant: action === 'delete' ? 'destructive' : 'secondary', onClick: () => beginAction(row, action), children: t(`actions.${action}`) }, action)) }) : null
...
export default {
  ...
  register(ctx) {
    ...
    ctx.i18n.register({
      en: {
        title: 'Skill Manage', subtitle: 'Inspect and maintain skills available to Hermes.', search: 'Search', searchPlaceholder: 'Search skills, descriptions, sources, statuses, or paths', refresh: 'Refresh', refreshing: 'Refreshing', retry: 'Retry', showDeleted: 'Show deleted built-ins', results: (shown, total) => `${shown} of ${total}`, allCategories: 'All categories', root: '(root)', details: 'Skill details', close: 'Close', noDescription: 'No description', recent: 'Recent actions', partial: 'Some skill data could not be loaded.', loadError: 'Skill inventory unavailable', unknownError: 'Unknown error', emptyTitle: 'No matching skills', emptyBody: 'Adjust or clear the active filters.', cancel: 'Cancel', confirm: 'Confirm', working: 'Working…', confirmPlaceholder: 'Type the exact skill name', success: (action, name) => `${action}: ${name}`, nav: 'Skills', open: 'Open Skill Manage',
        stats: { total: 'Total', builtin: 'Built-in', community: 'Community', local: 'Local' }, sources: { all: 'All', builtin: 'Built-in', 'hub-installed': 'Community', local: 'Local' }, statuses: { all: 'All statuses', enabled: 'Enabled', disabled: 'Disabled', deleted: 'Deleted' }, trust: { builtin: 'Built-in', official: 'Official', community: 'Community', local: 'Local' }, fields: { category: 'Category', source: 'Source', trust: 'Trust', status: 'Status', path: 'Path', identifier: 'Identifier', installed: 'Installed', updated: 'Updated' }, actions: { delete: 'Delete', reset: 'Reset', update: 'Update', restore: 'Restore' }, confirmTitle: { delete: 'Delete skill', reset: 'Reset skill' }, confirmBody: { delete: name => `This permanently removes the local files for ${name}. Type the exact skill name to continue.`, reset: name => `This replaces the local contents of ${name} with its source version. Type the exact skill name to continue.` }
      },
      zh: {
        title: '技能管理', subtitle: '查看并维护 Hermes 当前可用的技能。', search: '搜索', searchPlaceholder: '搜索技能、简介、来源、状态或路径', refresh: '刷新', refreshing: '刷新中', retry: '重试', showDeleted: '显示已删除内建', results: (shown, total) => `显示 ${shown} / ${total}`, allCategories: '全部分类', root: '（根目录）', details: '技能详情', close: '关闭', noDescription: '无简介', recent: '最近操作', partial: '部分技能数据加载失败。', loadError: '无法加载技能清单', unknownError: '未知错误', emptyTitle: '没有匹配的技能', emptyBody: '请调整或清除当前筛选条件。', cancel: '取消', confirm: '确认', working: '处理中…', confirmPlaceholder: '输入完整技能名', success: (action, name) => `${action}：${name}`, nav: '技能', open: '打开技能管理',
        stats: { total: '总数', builtin: '内建', community: '社区', local: '本地' }, sources: { all: '全部', builtin: '内建', 'hub-installed': '社区', local: '本地' }, statuses: { all: '全部状态', enabled: '启用', disabled: '停用', deleted: '已删除' }, trust: { builtin: '内建', official: '官方', community: '社区', local: '本地' }, fields: { category: '分类', source: '来源', trust: '信任', status: '状态', path: '路径', identifier: '标识', installed: '安装时间', updated: '更新时间' }, actions: { delete: '删除', reset: '重置', update: '更新', restore: '恢复' }, confirmTitle: { delete: '删除技能', reset: '重置技能' }, confirmBody: { delete: name => `这会永久删除 ${name} 的本地文件。请输入完整技能名继续。`, reset: name => `这会用来源版本覆盖 ${name} 的本地内容。请输入完整技能名继续。` }
      }
    })
